...
Usage:
  bash examples/tutor/scripts/eval_0818_personality.sh \
    [preflight|quick|subset|full|analyze]
...
Examples:
  ...
  # 192 seeded rows, proportionally stratified by MATH type x level.
  CUDA_VISIBLE_DEVICES=0,1,2,3 \
    bash examples/tutor/scripts/eval_0818_personality.sh subset

Phases:
  preflight  No output directory and no GPU process.
  quick      24 seeded test rows per cell; saves all traces.
  subset     192 type x level stratified rows per cell; saves error traces.
...
Common overrides:
  EVAL_RUN_DIR, BASE_PORT, EVAL_MAX_SAMPLES, EVAL_STRATIFIED_MAX_SAMPLES,
  EVAL_CONCURRENCY,
...
case "$PHASE" in
  preflight|quick)
    ...
    DEFAULT_STRATIFIED_MAX_SAMPLES=0
    ;;
  subset)
    DEFAULT_MAX_SAMPLES=0
    DEFAULT_STRATIFIED_MAX_SAMPLES=192
    ;;
  full)
    DEFAULT_MAX_SAMPLES=0
    DEFAULT_STRATIFIED_MAX_SAMPLES=0
...
EVAL_STRATIFIED_MAX_SAMPLES="${EVAL_STRATIFIED_MAX_SAMPLES:-$DEFAULT_STRATIFIED_MAX_SAMPLES}"
...
for integer_name in EVAL_MAX_SAMPLES EVAL_STRATIFIED_MAX_SAMPLES \
...
if (( EVAL_MAX_SAMPLES > 0 && EVAL_STRATIFIED_MAX_SAMPLES > 0 )); then
  printf 'Set only one of EVAL_MAX_SAMPLES and EVAL_STRATIFIED_MAX_SAMPLES.\n' >&2
  exit 2
fi
...
"$PYTHON" -B - "$RUN_DIR/manifest.json" "$PHASE" "$EVAL_MAX_SAMPLES" \
  "$EVAL_STRATIFIED_MAX_SAMPLES" "$EVAL_CONCURRENCY" "$GENERALIZE_REPLAYS" \
...
(
    output, phase, max_samples, stratified_max_samples, concurrency, replays,
    server_cap, caller_cap, save_traces, pair_count, base_port, episode_error_retries,
...
payload = {
    ...
    "eval_stratified_max_samples": int(stratified_max_samples),
...
printf '[run] phase=%s pairs=%s rows_per_cell=%s stratified_rows=%s output=%s\n' \
  "$PHASE" "$PAIR_COUNT" "$EXPECTED_ROWS" \
  "$EVAL_STRATIFIED_MAX_SAMPLES" "$RUN_DIR"
for ((pair_index = 0; pair_index < PAIR_COUNT; pair_index++)); do
