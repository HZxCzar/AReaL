...
# Exact 150-step comparison for the surface and all-ID gate-credit runs.
...
Usage:
  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
    bash examples/tutor/scripts/eval_0818_gate_credit_150.sh smoke

  CUDA_VISIBLE_DEVICES=0,1,2,3 \
    bash examples/tutor/scripts/eval_0818_gate_credit_150.sh subset

  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
    bash examples/tutor/scripts/eval_0818_gate_credit_150.sh full
...
STEP150=epoch3epochstep8globalstep149
...
export ADAPTER_SURFACE="${ADAPTER_SURFACE:-$CHECKPOINT_ROOT/$SURFACE_TRIAL/default/$STEP150}"
# The generic matrix calls the joint/all-ID teacher "full".
export ADAPTER_FULL="${ADAPTER_FULL:-$CHECKPOINT_ROOT/$FULL_TRIAL/default/$STEP150}"
...
for adapter in "$ADAPTER_SURFACE" "$ADAPTER_FULL"; do
  for required in adapter_model.safetensors adapter_config.json config.json; do
    if [[ ! -s "$adapter/$required" ]]; then
      printf 'Incomplete 150 checkpoint: %s is missing or empty.\n' \
        "$adapter/$required" >&2
      exit 1
    fi
  done
