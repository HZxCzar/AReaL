...
usage() {
  cat <<'EOF'
Usage:
  bash examples/tutor/scripts/eval_0901_step_demo_step1000_8gpu.sh [preflight|run|analyze]

Run the complete 1 x 7 matrix on exactly eight GPUs:
  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
    bash examples/tutor/scripts/eval_0901_step_demo_step1000_8gpu.sh run
...
Rebuild the matrix summary:
  EVAL_RUN_DIR=/path/printed/by/the/run \
    bash examples/tutor/scripts/eval_0901_step_demo_step1000_8gpu.sh analyze
...
LAUNCHER_SCRIPT="${MATRIX_LAUNCHER_SCRIPT:-examples/tutor/scripts/eval_0901_step_demo_step1000_8gpu.sh}"
EXPECTED_EXPLAIN_RATIO="${MATRIX_EXPECTED_EXPLAIN_RATIO:-1.0}"
OUTPUT_TAG="${MATRIX_OUTPUT_TAG:-step-demo-recovered-current-gates}"
...
TEACHERS=(step-demonstration)
declare -A TRIALS=(
  [step-demonstration]=20260906_133706_0901-preference-v3-reward-v4-step-demonstration-8gpu
)
