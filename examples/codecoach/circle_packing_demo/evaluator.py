...
try:
    with open({program_path!r}, "r", encoding="utf-8") as f:
        code = f.read()
    ...
    with open({(temp_file.name + ".results")!r}, "wb") as f:
...
def evaluate(program_path):
    ...
    try:
        ...
        combined_score = sum_radii if validity > 0 else 0.0

        print(
            f"Evaluation: valid={valid}, sum_radii={sum_radii:.6f}, "
...
if __name__ == "__main__":
    ...
    result["complexity"] = len(open(code_file, encoding="utf-8").read()) if os.path.exists(code_file) else 0
