...
# One 0818 personality-none teacher checkpoint against the ungated control and
# all six 0825 preference students. Runtime mode always uses four independent
# teacher/student server pairs, hence exactly eight GPUs.

usage() {
  cat <<'EOF'
Usage:
  bash examples/tutor/scripts/eval_0825_personality_8gpu.sh [preflight|run|analyze]

Examples:
  # Read-only: resolve the checkpoint, construct config semantics, and select the
  # exact 48-row stratified MATH subset. No server or output directory is created.
  bash examples/tutor/scripts/eval_0825_personality_8gpu.sh preflight

  # Full student rollout on exactly eight GPUs.
  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
    bash examples/tutor/scripts/eval_0825_personality_8gpu.sh run
  ...
  EVAL_RUN_DIR=/path/to/the/existing/run \
    bash examples/tutor/scripts/eval_0825_personality_8gpu.sh analyze

Defaults and useful overrides:
  EVAL_STRATIFIED_MAX_SAMPLES=48
  EVAL_CONCURRENCY=16
  BASE_PORT=35000
  ADAPTER_PATH=/explicit/checkpoint/path
...
every five seconds. Ctrl-C preserves these files; rerun with the same EVAL_RUN_DIR.
EOF
}
...
EVAL_CONFIG="$ROOT_DIR/examples/tutor/configs/math/0825/pilot/eval-all-preferences.yaml"
...
if (( EVAL_STRATIFIED_MAX_SAMPLES < 1 || EVAL_CONCURRENCY < 1 || \
  ...
  printf 'Samples/concurrency/timeouts must be positive and replays must remain 8.\n' >&2
  exit 2
fi
...
printf '[resolve] teacher trial=%s\n' "$TRIAL_NAME"
printf '[resolve] adapter=%s\n' "$ADAPTER_PATH"

PREFLIGHT_OUTPUT="$($PYTHON -B "$PREFLIGHT" \
  --config "$EVAL_CONFIG" \
...
RUN_DIR="${EVAL_RUN_DIR:-$TUTOR_FILEROOT/offline_eval/0825-personality/subset-$STAMP}"
...
validate_cell() {
  ...
  "$PYTHON" -B - "$output" "$student" "$preference" "$adapter" \
...
run_pair() (
  ...
  local adapter_alias="$pair_dir/served_adapters/$TEACHER_NAME"
  ...
  CUDA_VISIBLE_DEVICES="$teacher_gpu" setsid "$PYTHON" -m sglang.launch_server \
    ...
    --lora-paths "$adapter_alias" --max-loras-per-batch 1 --max-loaded-loras 1 \
    >"$teacher_log" 2>&1 &
  teacher_pid=$!
  ...
  if [[ "$SKIP_LIVENESS" == "0" ]]; then
    "$PYTHON" -B - "$teacher_url" "$TEACHER_MODEL" "$adapter_alias" <<'PY'
import sys
from examples.tutor.scripts.eval_checkpoints import probe

base_url, model, adapter = sys.argv[1:]
...
print(f"[liveness] active: {adapter}")
PY
  fi
...
printf '[run] pairs=4 rows_per_cell=%s cells=7 output=%s\n' \
  "$EXPECTED_ROWS" "$RUN_DIR"
...
if (( PENDING_COUNT > 0 )); then
  printf '[done-with-pending] %s episodes need backfill. Rerun with:\n' \
    "$PENDING_COUNT"
  printf 'CUDA_VISIBLE_DEVICES=%s EVAL_RUN_DIR=%q bash %q run\n' \
    "$CUDA_VISIBLE_DEVICES" "$RUN_DIR" \
    "examples/tutor/scripts/eval_0825_personality_8gpu.sh"
  exit 0
fi
