...
# Offline 1-teacher x 8-student evaluation for the saved 0818 full-student checkpoint.
#
...
usage() {
  cat <<'EOF'
Usage:
  bash examples/tutor/scripts/eval_0818_full_student.sh \
    [preflight|screen|full|analyze]

Examples:
  # Read-only: resolve checkpoints, load the real test split, and audit prompts.
  bash examples/tutor/scripts/eval_0818_full_student.sh preflight

  # Seeded 50-item screen. Two GPUs use one server pair; four use two pairs.
  CUDA_VISIBLE_DEVICES=0,1,2,3 bash \
    examples/tutor/scripts/eval_0818_full_student.sh screen

  # All filtered test items (currently 528).
  CUDA_VISIBLE_DEVICES=0,1,2,3 bash \
    examples/tutor/scripts/eval_0818_full_student.sh full

Phases:
  preflight  Default. No output directory, model server, or GPU job is created.
  screen     50 seeded test items x 1 full-student teacher x 8 students.
  full       Every filtered test item x 1 full-student teacher x 8 students.
  analyze    Rebuild the ID and OOD one-row summaries in EVAL_RUN_DIR.

Important environment variables:
  EVAL_RUN_DIR                Reuse/select an output root.
  GLOBAL_STEP_INDEX           Checkpoint directory index; 99 means completed step 100.
  ADAPTER_FULL                Explicit selected-step full-student adapter directory.
...
ANALYZER="$ROOT_DIR/examples/tutor/scripts/analyze_full_student_eval.py"
...
GLOBAL_STEP_INDEX="${GLOBAL_STEP_INDEX:-99}"
...
case "$PHASE" in
  preflight|screen) DEFAULT_MAX_SAMPLES=50 ;;
...
discover_full_adapter() {
  local explicit candidate
  explicit="${ADAPTER_FULL:-}"
  if [[ -n "$explicit" ]]; then
    printf '%s\n' "$(readlink -f "$explicit")"
    return
  fi
  candidate="$(
    find "$CHECKPOINT_ROOT" -type f \
      -path "*0818-full-8gpu/default/*globalstep${GLOBAL_STEP_INDEX}/adapter_model.safetensors" \
      -printf '%T@ %h\n' 2>/dev/null |
      sort -nr |
      head -1 |
      cut -d' ' -f2-
  ...
  if [[ -z "$candidate" ]]; then
    printf 'No 0818-full-8gpu globalstep%s adapter found under %s.\n' \
      "$GLOBAL_STEP_INDEX" "$CHECKPOINT_ROOT" >&2
    return 1
  fi
...
for teacher in "${TEACHERS[@]}"; do
  ADAPTERS["$teacher"]="$(discover_full_adapter)"
  for needed in adapter_model.safetensors adapter_config.json; do
    if [[ ! -f "${ADAPTERS[$teacher]}/$needed" ]]; then
      printf 'Incomplete %s adapter: %s\n' \
        "$teacher" "${ADAPTERS[$teacher]}" >&2
      exit 1
    fi
  done
done
...
run_semantic_preflight() {
  "$PYTHON" -B - \
    ...
    "$GLOBAL_STEP_INDEX" "${ADAPTERS[full]}" <<'PY'
import copy
import hashlib
import json
import sys
...
if len(adapter_paths) != 1:
    raise SystemExit("Expected exactly one full-student adapter path.")
adapter = adapter_paths[0]
if "0818-full-8gpu" not in adapter.as_posix():
    raise SystemExit(f"Expected the 0818-full-8gpu checkpoint, got {adapter}.")
if adapter.name.rsplit("globalstep", 1)[-1] != str(global_step_index):
    raise SystemExit(
        f"full: expected a globalstep{global_step_index} directory, got {adapter}."
    )
manifest = json.loads((adapter / "adapter_config.json").read_text())
if manifest.get("peft_type") != "LORA" or int(manifest.get("r", 0)) != 16:
    raise SystemExit("full: incompatible LoRA manifest.")
if Path(manifest.get("base_model_name_or_path", "")).resolve() != teacher_model_path.resolve():
    raise SystemExit("full: LoRA base model differs from actor.path.")
...
RUN_DIR="${EVAL_RUN_DIR:-$TUTOR_FILEROOT/offline_eval/0818-full-step${COMPLETED_TRAIN_STEP}/$STAMP}"
...
"$PYTHON" -B - "$RUN_DIR/manifest.json" "$PHASE" "$EVAL_MAX_SAMPLES" \
  ...
  "$BENCHMARK_CONCURRENCIES" "$COMPLETED_TRAIN_STEP" "$GLOBAL_STEP_INDEX" \
  "${ADAPTERS[full]}" <<'PY'
import json
import sys
...
payload = {
    ...
    "completed_train_step": int(completed_train_step),
    "checkpoint_directory_index": int(checkpoint_directory_index),
...
run_pair() (
  ...
  for teacher in "${TEACHERS[@]}"; do
    ...
    served["$teacher"]="$(readlink -f "$target")"
    # Keep the stable alias used by the evaluator's lora_path request.
    served_paths+=("$target")
  done
