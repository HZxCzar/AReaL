...
```bash
python -m areal.infra.launcher.slurm examples/werewolf/werewolf_grpo.py \
    --config examples/werewolf/werewolf_grpo.yaml \
    experiment_name=${EXPERIMENT_NAME} \
    trial_name=teacher-server \
    actor.path=${TEACHER_MODEL_PATH} \
    cluster.n_nodes=4 \
    actor.backend=fsdp:d8p1t4 rollout.backend=sglang:d8p1t4
```
...
python -m areal.infra.launcher.slurm examples/werewolf/werewolf_grpo.py \
    ...
    max_turns=${MAX_TURNS} \
    cluster.n_nodes=4 \
    actor.backend=fsdp:d8p1t2 rollout.backend=sglang:d4p2t2 \
    ${TEACHER_OBS_KWARGS}
```
...
python -m areal.infra.launcher.slurm examples/werewolf/werewolf_grpo.py \
    ...
    max_turns=${MAX_TURNS} \
    cluster.n_nodes=2 \
    actor.backend=fsdp:d8p1t1 rollout.backend=sglang:d8p1t1 \
    ${TEACHER_OBS_KWARGS}
```
