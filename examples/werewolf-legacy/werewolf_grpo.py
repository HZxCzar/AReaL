...
def main(args):
    ...
    opp_api_key = getattr(config, "opp_api_key", "") or ""
    teacher_api_key = getattr(config, "teacher_api_key", "") or ""

    opp_server_addrs = config.opp_server_addrs
    opp_rollout = None
    opp_api_model = None
    if opp_api_key and opp_api_key != "":
        opp_api_model = (
            _infer_api_model(opp_server_addrs)
            or os.getenv("AREAL_OPP_API_MODEL", "").strip()
            or None
        )
    elif opp_server_addrs != "":
    ...
    if teacher_api_key and teacher_api_key != "":
        teacher_api_model = (
            _infer_api_model(teacher_server_addrs)
            or os.getenv("AREAL_OPP_API_MODEL", "").strip()
            or None
        )
...
python -m areal.launcher.slurm examples/lite/werewolf_grpo.py \
    ...
    experiment_name=xmy-werewolf-comp-15-2 \
    trial_name=werewolf-gpt4o-vs4-villager-base3
