...
class PedagogicalRLWorkflow(RolloutWorkflow):
    ...
    def __init__(
        ...
        self.judge_client = judge_client or PedagogicalAPIClient(self.judge_model)
    ...
    async def _run_native_judges(
        ...
        *,
        ...
        async def call(prompt: str) -> list[str]:
            return await self.judge_client.generate(
    ...
    async def _rawbase_leak_check(
        self, episode: ClassroomEpisode, teacher_output: str
    ) -> tuple[bool, str | None, str, str]:
        visible = student_visible_text(teacher_output)
        ...
        try:
            outputs = await self.judge_client.generate(
                [
    ...
    async def _run_teacher_pre_solve(
        self, actor_client: Any, episode: ClassroomEpisode
    ) -> str | None:
        messages = [
    ...
    def _native_reward(self, episode: ClassroomEpisode) -> dict[str, float]:
        ...
        for message in episode.conversation:
            ...
            teacher_outputs.append(content)
        ...
        return {
            ...
            "total_reward": float(
                end_reward + thinking_reward + eoc_reward + length_reward
            ),
        }
        ...
        while not episode.should_stop_dialogue(
            ...
            if self.generation.leak_judge_mode == "turn":
                leaked, error, feedback, raw_output = await self._rawbase_leak_check(
                    episode, teacher_output
                )
                episode.leak_checks.append(
                    {
                        "teacher_turn": episode.teacher_turns,
                        "student_visible": student_visible_text(teacher_output),
                        "leaked": leaked,
                        "parse_error": error,
                        "feedback": feedback,
    ...
    async def _eval_episode(
        ...
        await self._run_native_judges(episode, stop_on_reject=False)
        episode.final_solutions = await self._student_turn(
            episode.student_messages(final=True),
            n=self.generation.number_student_attempts,
            max_tokens=self.generation.max_tokens_per_student_attempt,
        ...
        # GUIDED episodes have no initial attempt, so the delta pair is only
        # reported for ATTEMPTED ones and averages over that subset.
        initial_correct = (
            float(native_answer_correct(episode.initial_attempt, episode.answer))
            if episode.initial_attempt is not None
            else None
        )
        final_correct_math = 0.0
        if final_results:
            final_correct_math = sum(
                float(
        ...
        turn_leak_accuracy = reward if not turn_leak else 0.0
        native_leak_accuracy = reward if not native_leak else 0.0
        ...
        metrics = {
            ...
            "accuracy/raw": reward,
            "accuracy/turn_leak_gate": turn_leak_accuracy,
            "accuracy/pedagogical_rl_leak_gate": native_leak_accuracy,
            "pre_solved": 0.0,
            "solved": reward,
            "leaks": turn_leak,
            "turns": turns,
            "stop/leak": 0.0,
            "invalid_success_due_to_leak": float(bool(reward) and bool(turn_leak)),
            ...
            "leak_judges/disagree": float(bool(turn_leak) != bool(native_leak)),
            # Mirrored under ped_eval/* so the tutor arm, which runs this same
            # protocol from examples/tutor/workflow.py, is read off one metric.
            "ped_eval/final_correct": reward,
            "ped_eval/final_any_correct": final_any_correct,
            "ped_eval/final_correct_math": final_correct_math,
            "ped_eval/leaked": turn_leak,
            "ped_eval/turns": turns,
            "ped_eval/attempt_errors": 0.0,
        }
        # Leak rate among successful trajectories, under each arm's own leak
        # judge. Logged only when the trajectory succeeded, so the mean over the
        # eval set is P(leak | success) directly.
        if final_any_correct:
            metrics["leak_in_success/ped"] = float(native_leak)
            metrics["leak_in_success/tutor"] = float(turn_leak)
        if initial_correct is not None:
            metrics["ped_eval/initial_correct"] = initial_correct
            metrics["ped_eval/delta"] = reward - initial_correct
    ...
    async def arun_episode(self, engine: Any, data: dict[str, Any]) -> dict | None:
        ...
        if self.teacher_pre.enabled:
            episode.teacher_draft = await self._run_teacher_pre_solve(
                actor_client, episode
            )
            if episode.teacher_draft is None:
                self._safe_stats({"teacher_pre/rejected": 1.0})
                return None
        if is_eval:
            reward, metrics, details = await self._eval_episode(actor_client, episode)
            ...
            if self._cross_eval_selected(episode.problem):
                cross_metrics, cross_details = await self._run_cross_eval(
                    actor_client=actor_client, episode=episode
                )
            ...
            reward, components = await self._train_episode(actor_client, episode)
            solved = float(
                any(
                    native_answer_correct(solution, episode.answer)
                    for solution in episode.final_solutions
